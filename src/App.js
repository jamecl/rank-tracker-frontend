...
const RankTracker = () => {
  ...
  const fetchKeywords = async () => {
    try {
      ...
      if (data.success) {
        const formatted = data.data.map(k => ({
          id: k.keyword_id,
  ...
  const avgPosition = useMemo(() => {
    ...
    return (keywords.reduce((sum, kw) => sum + kw.position, 0) / keywords.length).toFixed(1);
  }, [keywords]);
        ...
        {showAddForm && (
          <div className="bg-white rounded-lg shadow-sm p-6 border border-slate-200 mb-6">
            ...
            <div className="flex gap-3">
              ...
              />
              <button onClick={handleAddKeyword} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                Add
              </button>
              <button onClick={() => setShowAddForm(false)} className="px-6 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200">
                Cancel
              </button>
            </div>
          </div>
